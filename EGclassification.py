...
def timing_decorator(func):
    def wrapper(*args, **kwargs):
        ...
        end_time = time.time()
        print(bcolors.OKGREEN + f'Function {func.__name__} took {convert_to_preferred_format(end_time - start_time)} to run.' + bcolors.ENDC)
        return result
    return wrapper
...
parser.add_argument('-m', "--method", dest='method', metavar='<method>', type=str, help='classifier name (default: LGBM, choice: RF|SVM|XGB|LGBM|MLP|LG|EXT|VC)', choices=['DUM', 'VC', 'RF', 'RUS', 'SVM', 'XGB', 'LGBM', 'EXT', 'MLP', 'WNN', 'LG'], default='LGBM', required=False)
...
def do_cv(nfolds, nclasses, seed):
  predictions = np.array([], dtype=int)
  ...
  for fold, (train_idx, test_idx) in enumerate(tqdm(kf.split(np.arange(len(X)), y), disable=args.batch, desc=bcolors.OKGREEN + f'{seed+1}-iter', total=nfolds)):
      ...
      scores = pd.concat([scores, pd.DataFrame([[roc_auc_score(test_y, ps) if nclasses <=2 else np.nan, accuracy_score(test_y, preds), balanced_accuracy_score(test_y, preds), 
...
#@timing_decorator
def classify(nfolds, repeat, jobs, nclasses):
  if jobs == 1:
    print(bcolors.OKCYAN + f'Running seq on 1 cpu...' + bcolors.ENDC)
    out = [do_cv(nfolds, nclasses, seed) for seed in range(repeat)]
  else: 
    if jobs == -1:
        print(bcolors.OKCYAN + f'Running par on {os.cpu_count()} cpus...' + bcolors.ENDC)
    else:
        print(bcolors.OKCYAN + f'Running par on {jobs} cpus...' + bcolors.ENDC)
    out = Parallel(n_jobs=jobs, prefer='threads')(delayed(do_cv)(nfolds, nclasses, seed) for seed in range(repeat))
  return out
...
out = classify(nfolds, args.repeat, args.jobs, nclasses)
...
df_scores.index=[f'{",".join([os.path.splitext(os.path.basename(file_path))[0] for file_path in args.inputfile])}']
df_scores['CM'] = [cma]
df_scores.columns = columns_names
ofile = sys.stdout if args.outfile is None else open(args.outfile, "a")
ofile.write(f'METHOD: {method}\n')
ofile.write(f'PROBL: {dict(df_label[labelname].value_counts())}\n')
ofile.write(f'INPUT: {" ".join(str(x) for x in args.inputfile)}\n')
ofile.write(f'LABEL: {args.labelfile} MODE: {"prob" if args.proba else "pred"}\n')
